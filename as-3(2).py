...
# def newtons_backward_difference(x_list: list, y_list: list, n: int, x: float, equispaced=True ) -> float:
#     h = x_list[1] - x_list[0]
#     # x= x0 + sh
#     # s = (x - x_list[0]) / h
#     # we would be creating a list of lists for the forward difference table: [[.], [.,.], [.,.,.], [.,.,.,.], ......so on]
...
#             row.append((difference_table[-1][i] - difference_table[-1][i-1]))
#         difference_table.append(row)
#     # table is created__________________________________________________________
...
#             # print(k, i)
#             term *= (x - x_list[i])/h
#         term *= (difference_table[k][-1])/math.factorial(k)
#         result += term
...
#     n = int(input("Enter degree of polynomial: "))
#     x = float(input("Enter x: "))
#     newtons_backward_difference(x_list, y_list, n, x)
